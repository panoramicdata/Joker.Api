...
/// </summary>
public class JokerApiIntegrationTests : TestBase<JokerApiIntegrationTests>, IDisposable
{
	...
	public JokerApiIntegrationTests()
	{
		...
		{
			Username = _configuration["JokerApi:Username"],
			Password = _configuration["JokerApi:Password"],
			ApiKey = _configuration["JokerApi:ApiKey"],
			Logger = _logger,
	...
	public async Task Login_WithValidCredentials_AuthenticatesSuccessfully()
	{
		...
		_ = response.StatusText.Should().NotBeNull();
		...
		else
		{
			...
			_ = (hasValidAuthSession || isSuccessfulStatus)
				.Should()
				.BeTrue($"Login failed - StatusCode: {response.StatusCode}, StatusText: {response.StatusText}, Result: {response.Result}");
